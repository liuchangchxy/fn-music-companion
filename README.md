...
4. 在安装向导中选择授权的两个文件夹：
   ...
   - **整理后文件夹（读写）**：存放整理后标准曲库的文件夹（建议与原始目录在同一存储卷以启用 Reflink 写时复制）；
5. 安装完成后，在飞牛桌面直接点击“飞牛音乐伴侣”图标即可进入嵌入式微应用控制台！

> 💡 **关于飞牛「文件夹访问权限」配置说明 (Folder Permissions Guide)**：
> - **权限配置路径**：在飞牛桌面打开「应用中心」->「已安装」-> 点击「飞牛音乐伴侣」卡片 ->「文件夹权限」->「添加文件夹」。
> - **子目录 vs 大文件夹**：
>   - **方式 A（推荐）**：分别添加「整理前」和「整理后」两个具体文件夹（整理前设为只读或读写，整理后必须设为读写）；
